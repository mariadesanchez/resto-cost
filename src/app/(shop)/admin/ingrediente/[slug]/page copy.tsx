...
import { createUpdateIngrediente, getMermas, IngredienteByProductId } from "@/actions";
...
export default function IngredienteForm({ ingrediente, params }: Props) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const productName = await getProductById('270683a2-8e29-40b2-808e-17e0f4b9f9a4');
        setProductName(productName?.title|| '');
      } catch (error) {
    ...
    >
      <div className="w-full">
        <div className="flex flex-col mb-2">
          <span className="text-lx font-bold mt-10 mb-10">{productName}</span>
          <select
            id="mermasSelect"
